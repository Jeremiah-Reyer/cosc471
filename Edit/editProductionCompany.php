...
<head>
	...
	<title>Edit Production Company</title>
	<style>
h1 {
        text-align:center;
    }
...
<p style="float:right"><a href="../Search/advancedSearch.php">Advanced Search</a></p>
	...
	<h1 class = "name">Edit Production Company</h1>
	
	<form action="editProductionCompany.php" method="post">

	<div style="float:left; margin-left: 5px; margin-right: 5px">
			<p>Production Company<br></p>
			<input name = pc_name required type = "text" placeholder = "Name"><br>
      ...
      <input type = "submit" value = "GO" name = "editProductionCompany" />
...
<?php
  if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editProductionCompany']))
{
	//get input data
	$name = $_POST['pc_name'];
	...
	if ($president == null)
	{
		$sql = "SELECT pname FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
	...
	if ($founder == null)
	{
		$sql = "SELECT fname FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$founder = $row['fname'];
	}
	if ($month == null)
	{
		$sql = "SELECT month FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$month = $row['month'];
	}
	if ($day == null)
	{
		$sql = "SELECT day FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$day = $row['day'];
	}
	if ($year == null)
	{
		$sql = "SELECT year FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$year = $row['year'];
	}
	if ($city == null)
	{
		$sql = "SELECT city FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$city = $row['city'];
	}
	if ($state == null)
	{
		$sql = "SELECT state FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$state = $row['state'];
	}
	if ($locations == null)
	{
		$sql = "SELECT offices FROM Production_company WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$locations = $row['offices'];
	}
			
	//update data
	$sql = "UPDATE Production_company 
			SET pname= '$president', fname='$founder', month='$month', day='$day', year='$year', city'$city', state='$state', offices='$locations'
			WHERE name='$name'";
