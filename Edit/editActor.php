...
<p style="float:right"><a href="../Search/advancedSearch.php">Advanced Search</a></p>
	...
	<div style="float:left; margin-left: 5px; margin-right: 5px">
      ...
      <input type = "submit" value = "GO" name = "editActor" />
...
<?php
  if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editActor']))
{
	//get input data
	$name = $_POST['a_name'];
	$age = $_POST['a_age'];
	...
	if ($age == null)
	{
		$sql = "SELECT age FROM Actors WHERE name='$name'";
		$results = $conn->query($sql);
	...
	if ($month == null)
	{
		$sql = "SELECT month FROM Actors WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$month = $row['month'];
	}
	if ($year == null)
	{
		$sql = "SELECT year FROM Actors WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$year = $row['year'];
	}
	if ($day == null)
	{
		$sql = "SELECT day FROM Actors WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$day = $row['day'];
	}
	if ($sex == null)
	{
		$sql = "SELECT sex FROM Actors WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$sex = $row['sex'];
	}
	if ($award == null)
	{
		$sql = "SELECT award FROM Actors WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$award = $row['award'];
	}
