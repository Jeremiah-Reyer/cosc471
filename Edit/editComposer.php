...
<p style="float:right"><a href="../Search/advancedSearch.php">Advanced Search</a></p>
	...
	<h1 class = "name">Edit Composer</h1>
	
	<form action ="editComposer.php" method="post">
	...
	<div style="float:left; margin-left: 5px; margin-right: 5px">
      ...
      <input type = "submit" value = "GO" name = "editComposer" />
...
<?php
  if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editComposer']))
{
	//get input data
	$name = $_POST['c_name'];
	$age = $_POST['c_age'];
	...
	if ($age == null)
	{
		$sql = "SELECT age FROM Composer WHERE name='$name'";
		$results = $conn->query($sql);
	...
	if ($month == null)
	{
		$sql = "SELECT month FROM Composer WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$month = $row['month'];
	}
	if ($year == null)
	{
		$sql = "SELECT year FROM Composer WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$year = $row['year'];
	}
	if ($day == null)
	{
		$sql = "SELECT day FROM Composer WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$day = $row['day'];
	}
	if ($sex == null)
	{
		$sql = "SELECT sex FROM Composer WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$sex = $row['sex'];
	}
	if ($award == null)
	{
		$sql = "SELECT award FROM Composer WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$award = $row['award'];
	}
