...
<head>
	...
	<title>Edit Screen Writer</title>
	<style>
h1 {
        text-align:center;
    }
...
<p style="float:right"><a href="../Search/advancedSearch.php">Advanced Search</a></p>
	...
	<h1 class = "name">Edit Screen Writer</h1>
	
	<form action="editScreenWriter.php" method="post">
	...
	<div style= "position:absolute; margin-left: 5px; margin-bottom: 5px">
			...
			<input type = "submit" value = "GO" name = "editScreenWriter" />
...
<?php
  if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editScreenWriter']))
{
	//get input data
	$name = $_POST['s_name'];
	$age = $_POST['s_age'];
	...
	if ($age == null)
	{
		$sql = "SELECT age FROM Screen_writter WHERE name='$name'";
		$results = $conn->query($sql);
	...
	if ($month == null)
	{
		$sql = "SELECT month FROM Screen_writter WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$month = $row['month'];
	}
	if ($year == null)
	{
		$sql = "SELECT year FROM Screen_writter WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$year = $row['year'];
	}
	if ($day == null)
	{
		$sql = "SELECT day FROM Screen_writter WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$day = $row['day'];
	}
	if ($sex == null)
	{
		$sql = "SELECT sex FROM Screen_writter WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$sex = $row['sex'];
	}
	...
	//update data
	$sql = "UPDATE Screen_writter 
			SET age= '$age', month='$month', year='$year', day='$day', sex='$sex'
			WHERE name='$name'";
