...
<p style="float:right"><a href="../Search/advancedSearch.php">Advanced Search</a></p>
	...
	<h1 class = "name">Edit Critic</h1>
	
	<form action="editCritic.php" method="post">

	<div style= "position:absolute; margin-left: 5px; margin-bottom: 5px">
			<p>Critic<br></p>
			<input name = c_name required type = "text" placeholder = "Name"><br>
			...
			<input name = movies_reviewed type = "number" placeholder = "Number of Movies Reviewed"><br>
			<input type = "submit" value = "GO" name = "editCritic" />
...
<?php
  if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editCritic']))
{
	//get input data
	$name = $_POST['c_name'];
	$age = $_POST['c_age'];
	...
	$year = $_POST['c_byear'];
	$day = $_POST['c_bday'];
	$moviesreviewed = $_POST['movies_reviewed'];
	...
	if ($age == null)
	{
		$sql = "SELECT age FROM Critic WHERE name='$name'";
		$results = $conn->query($sql);
	...
	if ($month == null)
	{
		$sql = "SELECT month FROM Critic WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$month = $row['month'];
	}
	if ($year == null)
	{
		$sql = "SELECT year FROM Critic WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$year = $row['year'];
	}
	if ($day == null)
	{
		$sql = "SELECT day FROM Critic WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$day = $row['day'];
	}
	if ($moviesreviewed == null)
	{
		$sql = "SELECT movies_reviewed FROM Critic WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$moviesreviewed = $row['movies_reviewed'];
	}
			
	//update data
	$sql = "UPDATE Critic 
			SET age= '$age', month='$month', year='$year', day='$day', movies_reviewed='$moviesreviewed'
			WHERE name='$name'";
