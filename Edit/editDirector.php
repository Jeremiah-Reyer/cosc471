...
<p style="float:right"><a href="../Search/advancedSearch.php">Advanced Search</a></p>
	...
	<h1 class = "name">Edit Director</h1>
	
	<form action = "editDirector.php" method="post">
	...
	<div style="float:left; margin-left: 5px; margin-right: 5px">
      ...
      <input type = "submit" value = "GO" name = "editDirector" />
...
<?php
  if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editDirector']))
{
	//get input data
	$name = $_POST['d_name'];
	$age = $_POST['d_age'];
	...
	if ($age == null)
	{
		$sql = "SELECT age FROM Director WHERE name='$name'";
		$results = $conn->query($sql);
	...
	if ($month == null)
	{
		$sql = "SELECT month FROM Director WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$month = $row['month'];
	}
	if ($year == null)
	{
		$sql = "SELECT year FROM Director WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$year = $row['year'];
	}
	if ($day == null)
	{
		$sql = "SELECT day FROM Director WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$day = $row['day'];
	}
	if ($sex == null)
	{
		$sql = "SELECT sex FROM Director WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$sex = $row['sex'];
	}
	if ($award == null)
	{
		$sql = "SELECT award FROM Director WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$award = $row['award'];
	}
