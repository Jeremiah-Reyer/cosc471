...
<p style="float:right"><a href="../Search/advancedSearch.php">Advanced Search</a></p>
	...
	<h1 class = "name">Edit Theater</h1>
	
	<form action="editTheater.php" method = "post">
	...
	<div style= "position:absolute; margin-left: 5px; margin-bottom: 5px">
      ...
      <input name = t_size type = "number" placeholder = "Size (sq ft)"><br>
			<input type = "submit" value = "GO" name = "editTheater" />
...
<?php
  if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editTheater']))
{
	//get input data
	$name = $_POST['t_name'];
	...
	if ($ownername == null)
	{
		$sql = "SELECT oname FROM Theater WHERE name='$name'";
		$results = $conn->query($sql);
	...
	if ($city == null)
	{
		$sql = "SELECT city FROM Theater WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$city = $row['city'];
	}
	if ($state == null)
	{
		$sql = "SELECT state FROM Theater WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$state = $row['state'];
	}
	if ($size == null)
	{
		$sql = "SELECT size FROM Theater WHERE name='$name'";
		$results = $conn->query($sql);
		$row = $results->fetch_assoc();
		$size = $row['size'];
	}
	...
	//update data
	$sql = "UPDATE Theater 
			SET oname= '$ownername', city='$city', state='$state', size='$size'
			WHERE name='$name'";
